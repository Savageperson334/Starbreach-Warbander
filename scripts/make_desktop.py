...
# Extract the four top-level builder pieces from the current responsive page.
war_pos = main_html.find('id="warbandSettings"')
ref_pos = main_html.find('id="factionReferencePanel"')
add_pos = main_html.find('class="panel pad stack add-panel"')
roster_pos = main_html.find('class="panel pad stack roster-panel"')
if min(war_pos, ref_pos, add_pos, roster_pos) < 0:
    raise RuntimeError('Expected builder panels were not found')

# Walk backwards to each section opening tag, then extract balanced section HTML.
def section_at(marker_pos):
    start = main_html.rfind('<section', 0, marker_pos)
    return extract_element(main_html, start, 'section')

war = section_at(war_pos)
ref = section_at(ref_pos)
add = section_at(add_pos)
roster = section_at(roster_pos)
...
# A visible route back to the responsive page. If the header markup changes, desktop still works without it.
